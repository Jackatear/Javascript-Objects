...
dropDown.addEventListener("click", () => {
  formDiv.classList.toggle("displayed");
});
...
function StoreBook(name, author, pages) {
  ...
  this.pages = pages.toLowerCase();
}

function inLib(book, lib) {
  for (let i = 0; i < lib.length; i++) {
    if (lib[i].name == book.name) return true;
  }
...
function addToList(book) {
  ...
  binButton.style.width = "50px";
  binButton.style.height = " 50px;";
  binButton.style.padding = "0px";
  ...
  tdName.textContent = toTitleCase(book.name);
  tdAuthor.textContent = toTitleCase(book.author);
  tdPages.textContent = toTitleCase(book.pages);
